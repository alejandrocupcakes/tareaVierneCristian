...
public class Procesos {
    ...
    public String calcularDefinitiva(double promedio) {
        return "El promedio total de los estudiantes es "+promedio;
    }
    ...
    public void registrarEnBD(String nombre, String documento, double nota1, double nota2, double nota3, double promedio) {
        if (obtenerEstudiante(documento) == null) {                        
            ...
            JOptionPane.showMessageDialog(null, "El estudiante está registrado", "Datos invalidos", JOptionPane.ERROR_MESSAGE);
        }
    }
    
    public boolean verificarValido(String nombre, String documento, double nota1, double nota2, double nota3) {
        if (nombre.length() < 1) {
            JOptionPane.showMessageDialog(null, "Se debe ingresar el nombre", "Datos invalidos", JOptionPane.ERROR_MESSAGE);
            return false;
        }
         if (documento.length() < 1) {
            JOptionPane.showMessageDialog(null, "Se debe ingresar el documento", "Datos invalidos", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        if (!(nota1 >= 0 && nota1 <= 5)) {
            JOptionPane.showMessageDialog(null, "La nota 1 debe ser del 1 a 5", "Datos invalidos", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        
        if (!(nota2 >= 0 && nota2 <= 5)) {
            JOptionPane.showMessageDialog(null, "La nota 2 debe debe ser del 1 a 5", "Datos invalidos", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        
        if (!(nota3 >= 0 && nota3 <= 5)) {
            JOptionPane.showMessageDialog(null, "La nota 3 debe debe ser del 1 a 5", "Datos invalidos", JOptionPane.ERROR_MESSAGE);
            return false;
        }
